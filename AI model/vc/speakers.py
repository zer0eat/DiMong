...
class SpeakerManager:
    ...
    def compute_speaker_embedding(self, wav_file: str):
        y, sr =librosa.load(path = wav_file,sr=16000)
        d_vector=torch.Tensor(y).cuda(device='cuda')
        d_vector = d_vector[None,:]
        return self.speaker_encoder.compute_embedding(d_vector)
    ...
    ################################################################################
    def compute_d_vector_from_clip(self, wav_file: Union[str, list]) -> torch.Tensor:
        ...
        if type(wav_file)=='str' :
            return self.compute_speaker_embedding(wav_file)
            
        else:
            length = len(wav_file)
            sum = self.compute_speaker_embedding(wav_file[0])
            
            for i in range(1,length):
                sum += self.compute_speaker_embedding(wav_file[i])
